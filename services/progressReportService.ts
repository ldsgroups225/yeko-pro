...
export const config = {
  ...
  async createLessonProgressReportConfig(
  ...
  async updateLessonProgressReportConfig(
    id: string,
  ...
  async deleteLessonProgressReportConfig(id: string): Promise<void> {
    const supabase = await createClient()
    await checkAuthAndGetSchoolId(supabase)
  ...
  async importLessonsProgressReportsConfig(reports: LessonProgressReportConfigInsert[]): Promise<{ successCount: number, errors: { row: number, error: string }[] }> {
    const supabase = await createClient()
    const schoolId = await checkAuthAndGetSchoolId(supabase)
