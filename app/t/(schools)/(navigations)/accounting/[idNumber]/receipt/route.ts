...
export async function GET(request: Request, { params }: { params: { idNumber: string } }) {
  try {
    ...
    const [school, histories, paymentPlan, student] = await Promise.all([
      ...
      fetchPaymentData(client, params.idNumber),
      fetchPaymentPlans(client, params.idNumber),
      fetchStudentData(client, params.idNumber, schoolId),
    ])
    ...
    response.headers.set(
      'Content-Disposition',
      `attachment; filename="invoice_${params.idNumber}.pdf"`,
    )
...
async function fetchPaymentData(client: SupabaseClient, studentIdNumber: string): Promise<IInvoicePaymentHistory[]> {
  ...
  // return data as unknown as IInvoicePaymentHistory[]
  return data.map(p => ({
    amount: p.amount,
    paidAt: p.paid_at!,
